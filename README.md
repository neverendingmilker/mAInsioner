...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Booster Links** — toggle, list/remove custom perk roles tracked per booster (auto-removed if they stop boosting) and inline edit to re-point a link to a different role; new links are added from Discord with `/boosterlink add` (same bot-role-above-the-linked-role check either way). A separate panel lists roles exempt from that auto-removal — the picker there is a multi-select, so several roles can be marked exempt in one submit. Each linked member's name also shows a **Mod** badge (reads the server's own `/modrole` setting, live) and/or an **OG/Fren** badge, if they currently hold whichever role was set as OG/Fren (there's no dashboard control for this anymore — the config card was removed, so it can only be changed directly in the database).
...
- **Reorder** (Admin-only) — the panel sections on that page ("cards") can be reordered by drag-and-drop, and each one also grows a resize grip on its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. Unlike the other items here it doesn't save on every click — switching it back off only saves if something was actually dragged or resized while it was active (no page reload needed, the page already reflects it). Dragging the resize grip changes width (column span) in whole steps, but height is completely free — it follows the cursor pixel-by-pixel instead of snapping to a fixed row size. The one exception: if a card's edge gets close to another visible card's edge while dragging, it magnetically snaps to match that exact height, making it easy to line two cards up without pixel-perfect aiming; keep dragging past that point and the snap releases on its own, so a card can never get stuck unable to shrink or grow further. Same-row cards are no longer forced to match each other's height by default. Shrink a card below what its own content needs and it scrolls internally instead of spilling past its edges. Placement (which column each card ends up in) is recomputed live as you drag or resize, always dropping the next card into whichever column currently has the most room — so different columns can end up with different numbers of cards (e.g. three narrower cards stacked on one side, one card resized tall next to them on the other), and you see exactly where everything lands as you go instead of guessing and reloading. While active, a "+ Slot vuoto" button also appears: it drops a blank, dashed placeholder card that drags and resizes exactly like a real one, letting you leave an intentional empty cell in a row or column instead of the layout always packing everything tight — a ✕ on the placeholder itself removes it again. The order and per-card size are saved to **this browser's own `localStorage`**, keyed by feature — not shared server-side, on purpose, so no one else's Admin session or device shows your layout; only an Admin's own browser can set it in the first place, and it doesn't follow that Admin to a different device.
