...
- **`/invites revoke`** `Mod` — Deletes a previously assigned invite (autocomplete over the active ones), including someone's self-made one — freeing up `create_self`'s one-invite-at-a-time limit is a Mod action, not something you can do for yourself.
...
- **Highlight** — just the on/off switch, nothing else. Every user's own words, channel mode and ignored-user list live only on Discord (`/highlight`) — nobody, not even an Admin, can see or change someone else's list from here. Switching it off stops matching and notifying server-wide without deleting anyone's saved words.
...
- **Starboard** — turn it on/off, see and manage every starboard (watch channel, post channel, reaction threshold, emoji list — including "any" — and content-type filter), add a new one or remove an existing one. Each one has a "Modifica" section pre-filled with all of its settings. There's also a "🔍" lookback section with the same options as `/starboard lookback` (message limit, or a since/until date range, one-off overrides for content type/emoji/threshold, extra channels to scan) — launching it doesn't block the page: the scan runs in the background of the same process, and a badge on the card updates live (messages scanned, how many got added) while it's going, without needing to reload.
...
- **Verification** — turn it on/off, set the give role for each type (Sub/Domme/Maledom), the shared remove role, the report channel and an extra allowed role, plus a separate "Ruoli sub" card for the set of roles `/verify sub` checks and its default-role fallback. A form lets you issue a verification for any member right from here — same role assign/remove, cross-type exclusivity and report-posting behavior as `/verify sub|domme|maledom` — and a recent-reports list lets you edit the Verification/Social fields of any report (no "only who issued it" restriction, same as `/verify edit`), live-updating the posted embed in Discord when you save.
