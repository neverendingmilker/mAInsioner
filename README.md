...
- **Mod access (opt-in per feature)**: Admins always have full access everywhere. A Mod only sees/reaches whichever feature pages an Admin has explicitly shared by switching a **"Solo Admin"** switch off, next to that feature's own on/off toggle (`src/dashboard/modAccess.js`, `dashboard_mod_access` table) — unshared features don't even show up in a Mod's sidebar. The switch's ON state (and its "Solo Admin" label) never changes: ON means mods are locked out, OFF means the feature is shared with them — it's the checked state that flips, not the wording. On a shared feature a Mod has the exact same read/write access as an Admin (add/edit/remove, etc.) — the one exception is the feature's on/off toggle and its base config (channel/role/schedule), which stay Admin-only even there, enforced centrally in `requireDashboardAccess` (`src/dashboard/middleware/requireAdmin.js`) rather than per-page. Tool pages (Ruoli & Permessi, Permessi per canale — see below) are always Admin-only, with no opt-in. Requires a Mod role to actually be configured via `/modrole` — with none set, only Administrators can log in, same as before this feature existed.
...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  - **Anime Night** — toggle, sessions grouped by watch date (each session = every anime watched that day), add titles to a new or existing date, and a "Modifica" section per session to replace its whole title list and/or move it to a new date. Removing a session deletes every title in it, not just one.
...
Every feature page's header shares the same controls (`src/dashboard/views/partials/featureToggle.ejs`), Admin-only where noted:

- **On/off** — a toggle switch, submitting the instant you flip it.
- **Solo Admin** (Admin-only) — see "Mod access" above; ON keeps the feature Admin-only, OFF also shares it with Mods. Label is fixed, only the switch position changes.
- **Modificabile** (Admin-only) — freezes that feature's own add/edit/remove/reorder forms without touching the on/off switch or base config (channel/role/schedule) when switched OFF; a locked feature keeps doing whatever it already does (QOTD keeps posting, Honeypot keeps trapping), only the dashboard's CRUD forms for its list are blocked, for Admin and Mod alike, until an Admin switches it back ON. Label is fixed ("Modificabile"), only the switch position changes.
- **Riordina card** (Admin-only) — the panel sections on that page ("cards") can be reordered by drag-and-drop; the switch toggles between browsing normally and rearranging them. Unlike the other switches here it doesn't save on every change — switching it back off only saves (and reloads the page) if a card was actually dragged while it was on, so moving several cards in a row doesn't reset you back to normal after each drop. The custom order is per server AND per feature, Admin-only to change, and applies to everyone who can open that page, Admin and Mod alike — any card never explicitly moved just keeps its default position.
