...
- **Riordina card** (Admin-only) — the panel sections on that page can be dragged into whatever order you like, and each one also gets a little grip in its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. It doesn't save on every change like the other three items above — switching it back off only saves (and reloads the page) if you actually dragged or resized something while it was active, so making several changes in a row doesn't kick you back to normal after each one. Cards live on a 3-column grid, and dragging the grip snaps a card wider/narrower (1 to 3 columns) or taller/shorter — so you can get three cards side by side at the same height, or one tall card next to two stacked ones that add up to exactly its height. Both the order and the sizes are per-server AND per-page, Admin-only to change, and shown to everyone who can open that page (Admin and Mod alike) — a panel you've never explicitly touched just keeps its default full-width spot.
