...
- **`/invites create`** — Makes a brand-new invite link and credits it to a specific user. Mods/Admin can do this for anyone, any number of times — handy for handing out personal "referral" links, e.g. to a booster or a partner. Everyone else can only credit *themselves*, and only once at a time — trying again while they already have one gets rejected until they `revoke` it, and they need the "Create Invite" permission themselves in whichever channel they pick (the bot having it isn't enough, to stop this being a backdoor into channels they couldn't normally invite people to). Optional max uses, and an expiry either as a relative `expires_in_hours` or an exact `expires_at` date/time (up to Discord's own 7-day cap). Already made an invite yourself and just want to credit it to someone? Pass `code` (the invite code or full link) instead of `channel` — the bot assigns your existing invite rather than making a new one; only joins from that point on are counted, since anything before wasn't logged.
...
- **`/invites revoke`** — Deletes a previously assigned invite (autocomplete over the active ones). Mods/Admin can revoke anyone's; everyone else only their own — the self-service undo for the one-invite-at-a-time limit on `create`.
