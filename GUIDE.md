...
- **Reorder** (Admin-only) — the panel sections on that page can be dragged into whatever order you like, and each one also gets a little grip in its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. It doesn't save on every change like the other items above — switching it back off only saves if you actually dragged or resized something while it was active (no reload needed, the page already shows it). Dragging the grip snaps a card wider/narrower in whole steps, but height is free — it follows the cursor exactly instead of jumping between fixed sizes. The exception: get a card's edge close to another visible card's edge while dragging and it magnetically locks onto that exact height, so lining two cards up doesn't need pixel-perfect dragging; keep pulling past that point and it lets go again on its own, so you can never get a card stuck at a size you can't change. Cards sitting in the same row are no longer forced to the same height by default. If you shrink a card smaller than its own content needs, it scrolls inside itself instead of the content spilling out. Cards reflow live as you drag or resize, always sliding the next one into whichever column has room — so it's easy to build something like three narrower cards stacked on one side and one card resized tall on the other, watching it snap into place as you go rather than guessing blind. While reordering, a "+ Slot vuoto" button shows up too — it adds a blank dashed placeholder you can drag and resize just like a real card, so you can leave a deliberate empty cell in a row or column instead of everything always packing tight; a ✕ on it removes it again. Order and size are saved to **your own browser's `localStorage`**, per feature — not shared with anyone else, on purpose, so another Admin (or you on a different device) never sees a layout you didn't set yourself.
...
- **Booster Links** — turn it on/off, see and manage every custom perk role tracked for a booster (it gets auto-removed if they stop boosting), remove an existing link, or re-point it to a different role from its "Modifica" section. Linking a *new* role happens from Discord with `/boosterlink add` (same bot-role-above-the-linked-role check as before — the dashboard no longer has its own "add" form). There's also a separate list of roles that are exempt from the auto-removal no matter what — its picker is a multi-select, so you can mark several roles exempt in one go instead of one at a time. Names in the linked-roles list also show a **Mod** badge (live from the server's `/modrole` setting) and/or an **OG/Fren** badge if they hold whichever role is set as OG/Fren — there's no dashboard control for picking that role anymore (the config card was removed), so it can only be changed directly in the database.
