...
- **Health check / keep-alive ping**: point Render's health check path at `/healthz`, not `/` — the root path now requires login. On Render itself the service also pings its own `/healthz` every 10 minutes (`src/dashboard/selfPing.js`, using the `RENDER_EXTERNAL_URL` Render sets automatically) so the free plan's ~15-minute idle sleep never kicks in — this is what actually keeps every node-cron scheduler (Bump Reminder's every-minute check included) ticking continuously; an external uptime ping (e.g. cron-job.org) still works too and is a reasonable backup, but is no longer required.
...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Highlight** — toggle only, nothing else: every word/phrase, channel mode and ignored-user list is strictly per-user and stays Discord-only (`/highlight`), by design not even an Admin can see or edit someone else's list. Turning the feature off here stops matching/notifying for the whole server without touching anyone's saved words.
  ...
  - **Starboard** — toggle, list/add/remove starboards (watch channel, post channel, reaction threshold, emoji list including "any", content-type filter), inline edit that can change every field at once. Lookback (rescanning a channel's history, same options as `/starboard lookback`: message limit or a since/until date range, one-off content-type/emoji/threshold overrides, extra channels) can also be launched from here — it starts in the background instead of blocking the request, with a live-updating progress badge (messages scanned/qualified) on the starboard's card while it runs.
  ...
  - **Verification** — toggle, configure the give role per type (Sub/Domme/Maledom), the shared remove role, the report channel and the extra allowed role, a separate "Ruoli sub" card for the default-sub-role fallback set, a form to issue a verification for any member (same role-assign/remove/mutual-exclusivity/report logic as `/verify sub|domme|maledom`), and a recent-reports list with an inline "Modifica" for the Verification/Social fields (same as `/verify edit`, no issuer restriction). Editing a report here also live-updates the posted embed message, exactly like the Discord edit flow.
...
`leaderboard`/`user`/`create_self` are open to everyone; `list`/`revoke` are `Mod`; `channel`/`create`/`disable` are `Admin`/`Mod`/`Admin` respectively. Tracks which invite each new member used and by whom it was created, so you know who's bringing people in.
...
- `create_self` `Everyone` — makes your own invite into the configured channel with default settings (unlimited uses, never expires); no options at all, no `user` to mix up: a separate command rather than a permission branch inside `create`, so it's clear upfront rather than something you find out from an error. Limited to one active self-made invite at a time — a second attempt is rejected until a Mod `revoke`s it for you. Also needs **Create Invite** for yourself in the configured channel (not just the bot's), so it can't be used as a backdoor into a channel you couldn't normally invite people to. For custom limits/expiry, or crediting an invite you already made elsewhere, ask a Mod to do it for you with `create`.
...
- `revoke code:<...>` `Mod` — deletes a previously assigned invite (autocomplete over active ones), including someone's self-made one — the undo for `create_self`'s one-at-a-time limit is a Mod action, not self-service.
