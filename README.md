...
- `/starboard add name:<...> watch_channel:<...> post_channel:<#channel> threshold:<1-1000> emojis:<...> [content_type] [exclude_channels]` — creates a new starboard. `watch_channel` takes one channel, or several separated by commas (as mentions or IDs), e.g. `watch_channel:#general,#memes` — up to 25; a channel can't also be the post channel. It also accepts the special value `all`, meaning **every channel in the server** — combine it with `exclude_channels:<...>` (same comma-separated format) to leave specific ones out; the post channel itself is always automatically excluded too, whether or not it's listed, so the bot never treats its own reposts as new candidate messages. `emojis` accepts one or more emojis (unicode or custom server emojis), separated by spaces or commas, e.g. `⭐` or `⭐ 🔥` — or the special value `any`, which counts a reaction with *any* emoji instead of specific ones (can't be combined with actual emojis). `content_type` is optional (see below), defaulting to "Any message".
