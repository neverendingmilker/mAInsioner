...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Booster Links** — toggle, list/add/remove custom perk roles tracked per booster (auto-removed if they stop boosting), inline edit to re-point a link to a different role, plus a separate list of roles exempt from that auto-removal. Linking a role still requires the bot's own role to sit above it, same check as `/boosterlink add`. Each linked member's name also shows a **Mod** badge (reads the server's own `/modrole` setting, live) and/or an **OG/Fren** badge, if they currently hold whichever role an Admin has picked as OG/Fren in a dedicated panel further down the page (picked from the server's real role list, not a fixed name/ID — none selected means the badge never shows).
...
- **Reorder** (Admin-only) — the panel sections on that page ("cards") can be reordered by drag-and-drop, and each one also grows a resize grip on its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. Unlike the other items here it doesn't save on every click — switching it back off only saves if something was actually dragged or resized while it was active (no page reload needed, the page already reflects it). Dragging the resize grip snaps a card to a narrower or wider column span and a taller or shorter row span in whole steps, never freely — same-row cards are no longer forced to match each other's height, so e.g. three cards can sit side by side each at their own natural height, or one wide/tall card can stand next to two stacked ones and come out exactly as tall as those two combined, if that's the size you actually drag it to. The order and per-card size are saved to **this browser's own `localStorage`**, keyed by feature — not shared server-side, on purpose, so no one else's Admin session or device shows your layout; only an Admin's own browser can set it in the first place, and it doesn't follow that Admin to a different device.
- **2 Cols / 3 Cols** (everyone) — chooses how many columns that page's card grid uses. Not Admin-only, since it's a personal viewing preference, not shared data — also saved to this browser's own `localStorage`, per feature.
- **Collapse** (everyone) — every card also gets a small collapse toggle in its corner, for cards that get too long to scan at a glance (e.g. a big list of saved birthdays); collapsing just hides everything below its heading. Same as the column choice, this is per-browser and available to Admin and Mod alike.
