...
- **Mod access (opt-in per feature)**: Admins always have full access everywhere. A Mod only sees/reaches whichever feature pages an Admin has explicitly shared with a **"Consenti anche ai Mod"** checkbox next to that feature's own on/off toggle (`src/dashboard/modAccess.js`, `dashboard_mod_access` table) — unshared features don't even show up in a Mod's sidebar. On a shared feature a Mod has the exact same read/write access as an Admin (add/edit/remove, etc.) — the one exception is the feature's on/off toggle and its base config (channel/role/schedule), which stay Admin-only even there, enforced centrally in `requireDashboardAccess` (`src/dashboard/middleware/requireAdmin.js`) rather than per-page. Tool pages (Ruoli & Permessi, Permessi per canale — see below) are always Admin-only, with no opt-in. Requires a Mod role to actually be configured via `/modrole` — with none set, only Administrators can log in, same as before this feature existed.
...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Question of the Day** — toggle, configure the posting channel/optional ping role and the schedule (a fixed daily time or every N hours), import questions from a published Google Sheet CSV link (link itself is saved on the dashboard, re-syncing skips duplicates) or add them manually, drag-and-drop the queue to reorder it (also the posting order), inline edit/remove per question, and a warning banner when the queue is exhausted (posting pauses automatically, resumes as soon as more questions are added — no reshuffle/loop). `/qotd post` and `/qotd status` are also available on Discord for a manual out-of-schedule post or a quick check.
...
Posts a question from a configured, reorderable queue on a schedule (fixed daily time or every N hours), optionally pinging a role. Config (channel, role, schedule, question list/reorder, Google Sheet import) is dashboard-only (sidebar → "Question of the Day") — this command only covers the two things worth doing from Discord itself.
- `disable` `Admin` — turns the feature on/off.
