...
Collects popular messages (by reaction count) and reposts them to a dedicated channel. A server can have several starboards, each with its own watch channel(s), post channel, threshold, emoji and content-type filter — e.g. one board watching `#general` and posting to `#starboard`, and a separate one watching `#memes` and posting only images to `#best-memes`. A board can watch several specific channels, or literally every channel in the server via `all`. `list` is open to everyone; every other subcommand requires **Administrator**.

- `/starboard add name:<...> watch_channel:<...> post_channel:<#channel> threshold:<1-1000> emojis:<...> [content_type] [exclude_channels]` — creates a new starboard. `watch_channel` takes one channel, or several separated by commas (as mentions or IDs) — both `watch_channel` and `exclude_channels` are autocompleted, suggesting channels/categories matching whatever's typed after the last comma and appending your pick to what's already there, so you can build the list one entry at a time instead of typing IDs by hand (autocomplete suggestions cap out around 4-5 entries due to Discord's own 100-character limit on a single suggestion — past that, finish the list by typing manually). E.g. `watch_channel:#general,#memes` — up to 25; a channel can't also be the post channel. **A category mention/ID expands to every text channel currently filed under it** — e.g. `watch_channel:#Staff Zone` (the category) watches every channel in that category, no need to list them individually; this works the same way in `exclude_channels`. It also accepts the special value `all`, meaning **every channel in the server** — combine it with `exclude_channels:<...>` (same comma-separated format, categories included) to leave specific ones out; the post channel itself is always automatically excluded too, whether or not it's listed, so the bot never treats its own reposts as new candidate messages. `emojis` accepts one or more emojis (unicode or custom server emojis), separated by spaces or commas, e.g. `⭐` or `⭐ 🔥` — or the special value `any`, which counts a reaction with *any* emoji instead of specific ones (can't be combined with actual emojis). `content_type` is optional (see below), defaulting to "Any message".
- `/starboard edit name:<...> [watch_channel] [post_channel] [threshold] [emojis] [content_type] [exclude_channels]` — updates any combination of an existing starboard's settings. The `name` option has autocomplete. Providing `watch_channel` **replaces the whole set** of watched channels (or switches to/from `all` mode), same rules as `add`. `exclude_channels` can also be provided **on its own**, without resending `watch_channel:all` — this updates just the exclusion list, and only works if the board is already in `all` mode (otherwise it's rejected with a clear error telling you to set `watch_channel:all` alongside it, or switch the board to `all` mode first). Providing `emojis` replaces the whole list, it doesn't add to it (also accepts `any`, same rules as above).
