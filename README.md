...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Starboard** — toggle, list/add/remove starboards (watch channel, post channel, reaction threshold, emoji list including "any", content-type filter), inline edit that can change every field at once. `/starboard lookback` (rescanning a channel's history) stays Discord-only — it can run long on channels with a huge backlog, not a fit for a synchronous dashboard request.
