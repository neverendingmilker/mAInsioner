...
- **Birthday** — turn it on/off, set the birthday role/removal timer/greeting channel (change just one and leave the others alone, same as `/birthday config`), see every saved birthday grouped by month with how many days until each one, add or update a birthday for any member (a dropdown, not typing a user ID), and remove one. If today happens to be the date you just saved, the role gets assigned and the greeting sent right away, same as doing it through `/birthday add`.
