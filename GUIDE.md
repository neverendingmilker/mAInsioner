...
- **Starboard** — turn it on/off, see and manage every starboard (watch channel, post channel, reaction threshold, emoji list — including "any" — and content-type filter), add a new one or remove an existing one. Each one has a "Modifica" section pre-filled with all of its settings. Rescanning a channel's past messages (`/starboard lookback`) stays Discord-only — it can take a while on a channel with a huge backlog, not something a dashboard request can wait on.
