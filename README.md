...
src/
  ...
  features/         <- "Business logic" layer: one folder per feature
    ...
    autoresponder/
      autoresponderManager.js     (validation, multi-emoji parsing, content-type filter matching, thread-to-parent-channel resolution, "pair mode" timing, passive per-message reaction)
...
Auto-reacts with one or more emojis to a chosen channel's messages, including messages posted in its threads (e.g. a forum's individual posts, or "room" threads under a shared parent) — no trigger word, unlike GoosePizza which only fires on a specific word. Each channel gets its own independent set of emojis and, optionally, its own content filter or pair-mode setting. All subcommands require the **Administrator** permission.

- `/autoresponder add channel:<#channel> emojis:<...> [require_attachment] [require_video_link] [require_x_link] [react_to_second_of_pair_within]` — sets (or replaces) the autoresponder for a channel. `emojis` accepts one or more unicode/custom server emojis, separated by spaces or commas (e.g. `🍕 🔥 ⭐`), up to 10 per channel; duplicates are silently deduped. The three boolean filters are all optional and off by default (reacts to every message, same as before this option existed): `require_attachment` matches messages with an image/gif/video attachment; `require_video_link` matches messages containing a video link (YouTube — `youtube.com/watch|shorts|live` and `youtu.be`); `require_x_link` matches messages linking an X/Twitter post, including the common embed-fixing mirror domains (`fxtwitter.com`, `vxtwitter.com`, `fixvx.com`, `fixupx.com`) alongside `x.com`/`twitter.com` themselves. Enabling more than one filter is an **OR**, not an AND — the message only needs to match one of the enabled filters to get a reaction. `react_to_second_of_pair_within:<1-30>` switches the channel into a separate **pair mode**: instead of reacting to every (filter-matching) message, it only reacts to a message that arrived less than that many seconds after the *previous* message in that same channel/thread — solo messages (no message shortly before them) get no reaction at all. This is meant for channels where something posts in two quick messages (e.g. a caption message immediately followed by an image message from another bot) and only the second should get the reaction. Content filters, if also set, still apply on top of the pair-timing check (both conditions must hold). Pair mode is the one case where messages from *other* bots are eligible for a reaction (normally every bot's messages are ignored) — this bot's own messages are still always excluded, in every mode, to avoid ever reacting to itself.
- `/autoresponder remove channel:<#channel>` — removes the autoresponder from a channel.
- `/autoresponder list` — shows every channel with an autoresponder configured, its emojis, and any active filter/pair mode.
