...
- `create_self [max_uses] [expires_in_hours or expires_at]` — same as `create`, but always for yourself, with no `user` option to mix up: a separate command rather than a permission branch inside `create`, so it's clear upfront rather than something you find out from an error. Limited to one active self-made invite at a time — a second attempt is rejected until the first is `revoke`d. Also needs **Create Invite** for yourself in the configured channel (not just the bot's), so it can't be used as a backdoor into a channel you couldn't normally invite people to.
