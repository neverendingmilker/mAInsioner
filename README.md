...
src/
  commands/         <- "Discord" layer: slash command definitions
    ...
    modaccess/
      index.js       (single-file command, no subcommands — broader mod-permission audit, incl. per-member channel overrides)
    incident/
...
`/verify config`, `/verify subroles` and `/verify disable` require the **Administrator** permission; `/verify sub`, `/verify domme`, `/verify maledom` and `/verify edit` require the server's configured Mod role (or Administrator), or the role configured via `/verify config allowedrole`.
...
- `/goosepizza edit name:<...> [trigger] [emoji] [mode]` — updates the word/phrase, emoji, and/or mode of an existing trigger. The `name` option has autocomplete — each suggestion shows that trigger's current word/phrase, emoji and response mode right in the label (e.g. `compleanno-trigger — "buon compleanno" 🎂 (reaction)`), so there's no need to check elsewhere first. A custom server emoji shows as a `:shortcode:` instead of the raw unrenderable `<:name:id>` mention, since autocomplete suggestions are plain text and don't display it as an actual image. If `mode` changes, every channel the trigger currently watches is re-checked for the new mode's required permission. Doesn't touch which channels it watches — use `/goosepizza channels` for that.
...
- `/mfaroles [ignore_bots]` — lists every role in the server that has at least one permission requiring 2FA for moderation, and which of those permissions it has. This is Discord's own fixed list of permissions blocked for a non-2FA account once the server turns on "Require 2FA for moderator actions" (Server Settings → Safety Setup): Administrator, Kick Members, Ban Members, Manage Server, Manage Roles, Manage Channels, Manage Messages, Manage Webhooks, Manage Threads, Mute Members, Deafen Members, Move Members. A role with Administrator implicitly has every other permission on the list too (that's how Discord permissions work), so it's shown as just "Administrator" rather than repeating all 12. Checks both each role's server-wide permissions AND any **per-channel permission override** that grants it an MFA-required permission it doesn't already have server-wide — flagged separately as "Channel overrides" with the channel name(s) and what's granted there (up to 10 channels shown per role before summarizing the rest), since that's the easiest kind of exposure to miss (a role that looks harmless server-wide, but was given Manage Messages on one specific channel and forgotten about). Optional `ignore_bots:true` skips roles Discord auto-created for a bot/app (and their channel overrides), for a cleaner view of just human-assignable roles. Member-specific overwrites (as opposed to role-based ones) aren't checked here — see `/modaccess` for that. No toggle in `/disablefeature`; it's a stateless read-only utility, not a feature with ongoing behavior. Requires the **Administrator** permission to run.
- `/modaccess [ignore_bots]` — a broader companion to `/mfaroles`: also flags permissions commonly *thought of* as moderator-level that Discord's 2FA requirement doesn't actually require — View Audit Log, Manage Nicknames, Manage Expressions (emoji/stickers/soundboard), Timeout Members — on top of the same MFA-required list. Unlike `/mfaroles`, it also checks per-channel overrides granted directly to an **individual person** (a member-type override), not just roles — those show up as their own entries (👤 name) separate from the role list (🎭 name), since a role-only audit would never catch a permission quietly granted to one specific person on one specific channel. Same `ignore_bots` option as `/mfaroles`, additionally skipping bot **accounts** with an individual channel override. Requires the **Administrator** permission to run; no toggle in `/disablefeature` (stateless read-only utility).
...
- `/slowmode add channel:<#channel> duration:<...>` — sets (or replaces) the limit for a channel. `duration` is a number followed by `s`/`m`/`h`/`d` (e.g. `12h`, `1d`, `3d`), minimum 1 minute. Requires **Manage Messages** or **Administrator**.
- `/slowmode remove channel:<...>` — removes the limit from a channel. The `channel` option is autocompleted, only showing channels that currently have a limit (with the cooldown shown), instead of every channel in the server. Requires **Manage Messages** or **Administrator**.
...
Admin only. Limits how many times each person can react per thread in a channel's threads — a spam/vote-brigading guard for reply threads, especially forum channels. The limit is **configurable per channel** (1–100, default 5) rather than fixed. Moderators (Manage Messages/Administrator) are always exempt from the limit.

- `/reactionlimit add channel:<#channel> [limit:1-100] [ignore_first_post:true|false]` — sets (or replaces) the limit for a channel's threads. `limit` defaults to 5 if not given. `ignore_first_post` (default `false`) excludes reactions on each thread's starter/first message from the count entirely — handy for e.g. a forum where the opening post is meant to collect votes/reactions freely, while still capping reaction spam in the replies underneath it.
- `/reactionlimit remove channel:<...>` — removes the limit from a channel. The `channel` option is autocompleted, only showing channels that currently have a limit configured, with the current limit shown in the suggestion (e.g. "#forum — limit: 5/thread"), instead of every channel in the server.
- `/reactionlimit edit channel:<...> [limit] [ignore_first_post]` — changes the settings for an already-configured channel. `channel` is autocompleted the same way as `remove`; anything not provided (including `limit`) keeps its current value.
