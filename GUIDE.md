...
- **`/invites revoke`** `Mod` (Everyone for your own) — Deletes a previously assigned invite (autocomplete over the active ones). Mods/Admin can revoke anyone's; everyone else only their own — the self-service undo for `create_self`'s one-invite-at-a-time limit.
