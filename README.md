...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  ...
  - **Booster Links** — toggle, list/add/remove custom perk roles tracked per booster (auto-removed if they stop boosting), inline edit to re-point a link to a different role, plus a separate list of roles exempt from that auto-removal. Linking a role still requires the bot's own role to sit above it, same check as `/boosterlink add`.
...
Every feature page's header shares the same controls (`src/dashboard/views/partials/featureToggle.ejs`), rendered as one connected button group — colored (active) or the default muted color (inactive), driven by CSS `:has(input:checked)` on a hidden checkbox behind each item, no JS needed for the styling itself. Admin-only items where noted:

- **Attivo** — the feature's own on/off, submits the instant you click it.
- **Solo Admin** (Admin-only) — see "Mod access" above; colored/active keeps the feature Admin-only, uncolored/inactive also shares it with Mods. Label is fixed, only the color changes.
- **Modificabile** (Admin-only) — freezes that feature's own add/edit/remove/reorder forms without touching Attivo or base config (channel/role/schedule) when switched to inactive; a locked feature keeps doing whatever it already does (QOTD keeps posting, Honeypot keeps trapping), only the dashboard's CRUD forms for its list are blocked, for Admin and Mod alike, until an Admin makes it active again. Label is fixed ("Modificabile"), only the color changes.
- **Riordina card** (Admin-only) — the panel sections on that page ("cards") can be reordered by drag-and-drop, and each one also grows a resize grip on its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. Unlike the other three items here it doesn't save on every click — switching it back off only saves (and reloads the page) if something was actually dragged or resized while it was active, so making several changes in a row doesn't reset you back to normal after each one. Cards sit in a 3-column grid — dragging the resize grip snaps a card to a narrower or wider column span (1–3 columns) and a taller or shorter row span, so e.g. three cards can sit side by side at equal height, or one wide card can stand next to two stacked ones and come out exactly as tall as those two combined. The order and sizes are per server AND per feature, Admin-only to change, and apply to everyone who can open that page, Admin and Mod alike — any card never explicitly moved/resized just keeps its default full-width position.

Every per-item row across these lists (a channel, a link, a session, …) shares the same two icon buttons in its own header: a pencil (✎) for "Modifica", opening a small popover with that item's edit form right under the button (a native `<details>`/`<summary>`, no JS), and a red X (✕) for "Rimuovi", always the same color regardless of the item. Where an item has nothing to edit (just a one-off remove/revoke action) only the red X shows.
...
Posts a question from a configured, reorderable queue on a schedule (fixed daily time or every N hours), optionally pinging a role. Config (channel, role, schedule, question list/reorder) is dashboard-only (sidebar → "Question of the Day") — this command only covers the two things worth doing from Discord itself.
- `disable` `Admin` — turns the feature on/off.
- `post` `Admin` — posts the next question in the queue right now, regardless of the schedule.
- `status` `Admin` — shows the current configuration, queue size, and the next question preview.
