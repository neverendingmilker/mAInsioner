...
- **Reorder** (Admin-only) — the panel sections on that page can be dragged into whatever order you like, and each one also gets a little grip in its bottom-right corner; clicking toggles between browsing normally and rearranging/resizing them. It doesn't save on every change like the other items above — switching it back off only saves if you actually dragged or resized something while it was active (no reload needed, the page already shows it). Dragging the grip snaps a card wider/narrower or taller/shorter in whole steps, never freely — cards sitting in the same row are no longer forced to the same height, so you can get e.g. three cards side by side each at their own natural height, or one tall card next to two stacked ones that add up to exactly its height, if you actually resize it that way. Order and size are saved to **your own browser's `localStorage`**, per feature — not shared with anyone else, on purpose, so another Admin (or you on a different device) never sees a layout you didn't set yourself.
- **2 Cols / 3 Cols** (everyone, Admin and Mod) — how many columns that page's cards use — just a personal viewing choice, saved the same per-browser way.
- **Collapse** (everyone, Admin and Mod) — a small toggle on every card to fold it down to just its heading, handy for a card that's grown too long (e.g. a big saved-birthdays list). Also per-browser, not shared.
