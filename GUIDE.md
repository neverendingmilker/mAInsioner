...
Every feature page's header shares the same controls, top-right, rendered as one connected button group — colored when active, the default muted color when not:

- **Attivo** — the feature's own on/off, applies the moment you click it, same control on every feature page.
- **Solo Admin** (Admin-only) — see "Admin vs. Mod" above; colored/active keeps the feature Admin-only, uncolored/inactive also shares it with Mods. The label never changes, only the color.
- **Modificabile** (Admin-only) — when made inactive, freezes that page's own add/edit/remove/reorder forms without touching Attivo or base config (channel/role/schedule); a locked feature keeps doing whatever it already does (QOTD keeps posting, Honeypot keeps trapping), only the dashboard's own CRUD forms for its list stop working, for Admin and Mod alike, until an Admin makes it active again. The label never changes, only the color.
- **Riordina card** (Admin-only) — the panel sections on that page can be dragged into whatever order you like; clicking toggles between browsing normally and rearranging them. It doesn't save on every change like the other three items above — switching it back off only saves (and reloads the page) if you actually dragged a card while it was active, so rearranging several cards in a row doesn't kick you back to normal after each drop. The order is per-server AND per-page, Admin-only to change, and shown to everyone who can open that page (Admin and Mod alike) — a panel you've never explicitly moved just keeps its default position.

- **Anime Night** — turn it on/off, sessions listed grouped by the date they were watched (a "session" = every anime watched that day), each with a "Modifica" section to replace the whole title list and/or move it to a different date. Add new titles to today (or a chosen date) from the form at the bottom — landing on a date that already has a session just adds to it. Removing a session removes every title in it at once. EXPERIMENTAL, only here for now: its two cards sit side by side in two columns, and while "Riordina card" above is switched on, each one also gets a little grip in its bottom-right corner — drag it to resize that card's width and height, same as resizing a textarea in a browser. Switching "Riordina card" back off removes the grip but keeps the size you left it at; nothing about card size is saved between page loads though, so a refresh puts everything back to the default split. Still works together with dragging the two cards themselves to swap which is on the left/right.
