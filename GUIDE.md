...
Every page's on/off switch, top-right, is a toggle switch that applies the moment you flip it — same control on every feature page.

The sidebar's feature list can be reordered by dragging entries around; a lock button in its header switches between normal browsing and rearranging. The custom order is per-server, Admin-only to change, saved on the dashboard and shown to everyone who can see that sidebar (Admin and Mod alike) — a feature you've never explicitly moved just keeps its default alphabetical spot.
