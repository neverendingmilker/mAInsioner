...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  - **Anime Night** — toggle, sessions grouped by watch date (each session = every anime watched that day), add titles to a new or existing date, and a "Modifica" section per session to replace its whole title list and/or move it to a new date. Removing a session deletes every title in it, not just one. EXPERIMENTAL, piloted here only: the two cards ("Sessioni" and "Aggiungi anime") sit side by side in a two-column layout instead of stacked. While "Riordina card" (see below) is on, each card also grows a native browser resize grip on its bottom-right corner — drag it to resize that card's own width AND height (plain CSS `resize: both`, no custom drag code); turning "Riordina card" back off removes the grip but keeps whatever size you left it at. Sizes aren't saved anywhere yet — they reset the next time the page loads.
...
- **Riordina card** (Admin-only) — the panel sections on that page ("cards") can be reordered by drag-and-drop; clicking toggles between browsing normally and rearranging them. Unlike the other three items here it doesn't save on every click — switching it back off only saves (and reloads the page) if a card was actually dragged while it was active, so moving several cards in a row doesn't reset you back to normal after each drop. The custom order is per server AND per feature, Admin-only to change, and applies to everyone who can open that page, Admin and Mod alike — any card never explicitly moved just keeps its default position.
