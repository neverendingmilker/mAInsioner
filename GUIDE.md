...
- **`/autoresponder add`** — Sets (or replaces) the autoresponder for a channel: one or more emojis, separated by spaces or commas. Three optional filters (off by default, reacts to everything): only react if the message has an **attachment** (image/gif/video), only if it **links a video** (YouTube and similar), only if it **links an X/Twitter post** (including fxtwitter/vxtwitter/fixvx/fixupx mirrors). Turn on more than one and it reacts if the message matches *any* of them. There are also two mutually-exclusive special modes (pick at most one per channel):
  - **Pair mode** (`react_to_second_of_pair_within`, in seconds) — reacts only to the *second* of two messages posted less than N seconds apart, ignoring solo messages entirely; also lets other bots' messages get reacted to (normally all bots are ignored).
  - **Redirect mode** (`redirect_to_bot_id` + `redirect_window_seconds`) — waits for a *specific* bot to post in the same channel within the window; if it does, the reaction goes on **its** message instead of the original poster's. If that bot doesn't post in time, the original message gets the reaction as a fallback. Useful when a specific bot reliably reposts the "real" content shortly after someone's message.
