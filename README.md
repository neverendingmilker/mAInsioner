...
- **Per-feature config pages**: features get their own dashboard page one at a time — see `src/dashboard/sidebarData.js`'s `FEATURE_PAGES` map for which ones have one so far. A feature without an entry there just shows in an Admin's sidebar as "coming soon" (Mods never see unshared or page-less features listed at all).
  - **Anime Night** — toggle, sessions grouped by watch date (each session = every anime watched that day), add titles to a new or existing date, and a "Modifica" section per session to replace its whole title list and/or move it to a new date. Removing a session deletes every title in it, not just one. EXPERIMENTAL, piloted here only: the two cards ("Sessioni" and "Aggiungi anime") sit in a two-column layout instead of stacked, with a resizable divider between them (`src/dashboard/public/cardResize.js`, `dashboard_card_layout` table) — Admin-only to drag, the saved width split shows for Admin and Mod alike. Coexists with the regular card drag-reorder (see "Riordina card" below): the divider is always explicitly grid-placed in the middle column so reordering the two cards never displaces it.
