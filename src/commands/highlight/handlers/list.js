...
const MODE_LABEL = { exclude: 'Ignored channels', include: 'Only these channels' };

async function handleList(interaction) {
  const [words, channelIds, channelMode, ignoredUserIds] = await Promise.all([
    ...
    highlightManager.getChannelMode(interaction.guildId, interaction.user.id),
  ...
  const channelsText = channelIds.length > 0 ? channelIds.map((id) => `<#${id}>`).join(', ') : '*(none)*';
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `${MODE_LABEL[channelMode]} (mode: ${channelMode})`, value: channelsText },
