...
Every feature page's header shares the same controls, top-right:

- **On/off** — a toggle switch that applies the moment you flip it, same control on every feature page.
- **Solo Admin** (Admin-only) — see "Admin vs. Mod" above; ON keeps the feature Admin-only, OFF also shares it with Mods. The label never changes, only the switch position.
- **Modificabile** (Admin-only) — when switched OFF, freezes that page's own add/edit/remove/reorder forms without touching the on/off switch or base config (channel/role/schedule); a locked feature keeps doing whatever it already does (QOTD keeps posting, Honeypot keeps trapping), only the dashboard's own CRUD forms for its list stop working, for Admin and Mod alike, until an Admin switches it back ON. The label never changes, only the switch position.
- **Riordina card** (Admin-only) — the panel sections on that page can be dragged into whatever order you like; the switch toggles between browsing normally and rearranging them. It doesn't save on every change like the other switches above — switching it back off only saves (and reloads the page) if you actually dragged a card while it was on, so rearranging several cards in a row doesn't kick you back to normal after each drop. The order is per-server AND per-page, Admin-only to change, and shown to everyone who can open that page (Admin and Mod alike) — a panel you've never explicitly moved just keeps its default position.
