...
Every page's on/off switch, top-right, is now two radio buttons ("Attivo"/"Disattivo") that apply the moment you click one — same control on every feature page.
...
There's also a separate "Strumenti" section in the sidebar, below the feature list, for tools with no on/off switch — they're not features with saved settings, just a live query recomputed every time you open the page, same as running the equivalent slash command.
